...
let map_anon_choice_pat_func_6731ab8 (env : env) (x : CST.anon_choice_pat_func_6731ab8) =
  (match x with
  | `Pat_func x -> map_pat_func env x
  | `Pat_const x -> map_pat_const env x
  )
...
let map_namespace_name_as_prefix (env : env) (x : CST.namespace_name_as_prefix) =
  ...
  | `Opt_BSLASH_name_name_BSLASH (v1, v2, v3) ->
      ...
      todo env (v1, v2, v3)
  | `Pat_name_BSLASH (v1, v2) ->
      let v1 = map_pat_name env v1 in
      let v2 = (* "\\" *) token env v2 in
      todo env (v1, v2)
  | `Pat_name_opt_BSLASH_name_name_BSLASH (v1, v2, v3, v4) ->
      let v1 = map_pat_name env v1 in
      let v2 =
        (match v2 with
        | Some tok -> (* "\\" *) token env tok
...
let map_relative_scope (env : env) (x : CST.relative_scope) =
  ...
  | `Pat_static x -> map_static_modifier env x
  )
...
let map_namespace_use_group_clause (env : env) ((v1, v2, v3) : CST.namespace_use_group_clause) =
  let v1 =
    (match v1 with
    | Some x -> map_anon_choice_pat_func_6731ab8 env x
    | None -> todo env ())
  in
  let v2 = map_namespace_name env v2 in
  let v3 =
...
and map_object_creation_expression (env : env) (x : CST.object_creation_expression) =
  ...
  | `New_choice_qual_name_opt_args (v1, v2, v3) ->
      ...
      in
      todo env (v1, v2, v3)
  | `New_pat_class_opt_args_opt_base_clause_opt_class_inte_clause_decl_list (v1, v2, v3, v4, v5, v6) ->
      let v1 = (* "new" *) token env v1 in
...
and map_statement (env : env) (x : CST.statement) =
  ...
  | `If_stmt (v1, v2, v3) ->
      ...
      let v3 =
        ...
        | `Choice_empty_stmt_rep_else_if_clause_opt_else_clause (v1, v2, v3) ->
            ...
            in
            todo env (v1, v2, v3)
        | `Colon_blk_rep_else_if_clause_2_opt_else_clause_2_pat_endif_choice_auto_semi (v1, v2, v3, v4, v5) ->
  ...
  | `While_stmt (v1, v2, v3) ->
      ...
      let v3 =
        ...
        | `Colon_blk_pat_endw_choice_auto_semi (v1, v2, v3) ->
            let v1 = map_colon_block env v1 in
  ...
  | `For_stmt (v1, v2, v3, v4, v5, v6, v7, v8, v9) ->
      ...
      let v9 =
        ...
        | `Choice_auto_semi x -> map_semicolon env x
        | `Choice_empty_stmt x -> map_statement env x
        | `COLON_rep_choice_empty_stmt_pat_endfor_choice_auto_semi (v1, v2, v3, v4) ->
            let v1 = (* ":" *) token env v1 in
            let v2 = List.map (map_statement env) v2 in
  ...
  | `Fore_stmt (v1, v2, v3, v4, v5, v6, v7) ->
      ...
      let v7 =
        ...
        | `Choice_auto_semi x -> map_semicolon env x
        | `Choice_empty_stmt x -> map_statement env x
        | `Colon_blk_pat_endf_choice_auto_semi (v1, v2, v3) ->
            let v1 = map_colon_block env v1 in
  ...
  | `Decl_stmt (v1, v2, v3, v4, v5) ->
      ...
      let v5 =
        (match v5 with
        | `Choice_empty_stmt x -> map_statement env x
        | `COLON_rep_choice_empty_stmt_pat_endd_choice_auto_semi (v1, v2, v3, v4) ->
            let v1 = (* ":" *) token env v1 in
            let v2 = List.map (map_statement env) v2 in
...
and map_switch_block (env : env) (x : CST.switch_block) =
  ...
  | `LCURL_rep_choice_case_stmt_RCURL (v1, v2, v3) ->
      ...
      let v3 = (* "}" *) token env v3 in
      todo env (v1, v2, v3)
  | `COLON_rep_choice_case_stmt_pat_ends_choice_auto_semi (v1, v2, v3, v4) ->
      let v1 = (* ":" *) token env v1 in
      let v2 =
        List.map (map_anon_choice_case_stmt_f1b35bc env) v2
      in
