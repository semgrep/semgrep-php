...
let map_pat_35b755d (env : env) (tok : CST.pat_35b755d) =
...
let map_pat_1b8b911 (env : env) (tok : CST.pat_1b8b911) =
  (* pattern [dD][eE][fF][aA][uU][lL][tT] *) token env tok

let map_pat_30717f3 (env : env) (tok : CST.pat_30717f3) =
  (* pattern [uU][sS][eE] *) token env tok

let map_pat_40e76f1 (env : env) (tok : CST.pat_40e76f1) =
  (* pattern [eE][cC][hH][oO] *) token env tok

let map_pat_8b02371 (env : env) (tok : CST.pat_8b02371) =
...
let map_pat_7beed81 (env : env) (tok : CST.pat_7beed81) =
  (* pattern [cC][oO][nN][sS][tT] *) token env tok

let map_pat_66accd2 (env : env) (tok : CST.pat_66accd2) =
...
let map_pat_feb296c (env : env) (tok : CST.pat_feb296c) =
  (* pattern [rR][eE][qQ][uU][iI][rR][eE][__][oO][nN][cC][eE] *) token env tok

let map_pat_5295da4 (env : env) (tok : CST.pat_5295da4) =
...
let map_pat_e8f6be6 (env : env) (tok : CST.pat_e8f6be6) =
...
let map_pat_2bdb730 (env : env) (tok : CST.pat_2bdb730) =
...
let map_pat_de4334b (env : env) (tok : CST.pat_de4334b) =
...
let map_pat_25e0188 (env : env) (tok : CST.pat_25e0188) =
...
let map_pat_b10beb6 (env : env) (tok : CST.pat_b10beb6) =
...
let map_pat_7f9f785 (env : env) (tok : CST.pat_7f9f785) =
  (* pattern [mM][aA][tT][cC][hH] *) token env tok

let map_pat_0b47e00 (env : env) (tok : CST.pat_0b47e00) =
  (* pattern [eE][nN][dD][sS][wW][iI][tT][cC][hH] *) token env tok

let map_pat_bb9603f (env : env) (tok : CST.pat_bb9603f) =
  (* pattern [eE][nN][dD][dD][eE][cC][lL][aA][rR][eE] *) token env tok

let map_pat_5641180 (env : env) (tok : CST.pat_5641180) =
...
let map_pat_1d5f5b3 (env : env) (tok : CST.pat_1d5f5b3) =
...
let map_pat_1e9d49b (env : env) (tok : CST.pat_1e9d49b) =
...
let map_pat_7f63d6a (env : env) (tok : CST.pat_7f63d6a) =
  (* pattern [gG][oO][tT][oO] *) token env tok

let map_pat_174c3a5 (env : env) (tok : CST.pat_174c3a5) =
...
let map_pat_fb2c292 (env : env) (tok : CST.pat_fb2c292) =
  (* pattern [fF][nN] *) token env tok

let map_pat_3e83199 (env : env) (tok : CST.pat_3e83199) =
...
let map_namespace_aliasing_clause (env : env) ((v1, v2) : CST.namespace_aliasing_clause) =
  let v1 = map_pat_79b8891 env v1 in
  let v2 =
    (* pattern [_a-zA-Z\u00A1-\u00ff][_a-zA-Z\u00A1-\u00ff\d]* *) token env v2
  in
  todo env (v1, v2)
...
let map_anon_choice_pat_174c3a5_81b85de (env : env) (x : CST.anon_choice_pat_174c3a5_81b85de) =
  (match x with
  | `Pat_174c3a5 x -> map_pat_174c3a5 env x
  | `Pat_7beed81 x -> map_pat_7beed81 env x
  )
...
let map_namespace_name_as_prefix (env : env) (x : CST.namespace_name_as_prefix) =
  ...
  | `Pat_1e9d49b_BSLASH (v1, v2) ->
      let v1 = map_pat_1e9d49b env v1 in
      let v2 = (* "\\" *) token env v2 in
      todo env (v1, v2)
...
let map_anonymous_function_use_clause (env : env) ((v1, v2, v3, v4, v5, v6, v7) : CST.anonymous_function_use_clause) =
  let v1 = map_pat_30717f3 env v1 in
  let v2 = (* "(" *) token env v2 in
  let v3 =
    (match v3 with
    | Some tok -> (* "&" *) token env tok
...
let map_modifier (env : env) (x : CST.modifier) =
  ...
  | `Static_modi x -> map_static_modifier env x
  | `Final_modi x -> map_final_modifier env x
  | `Abst_modi x -> map_abstract_modifier env x
  )
...
let map_relative_scope (env : env) (x : CST.relative_scope) =
  ...
  | `Pat_068a1b3 x -> map_static_modifier env x
  )
...
and map_array_creation_expression (env : env) (x : CST.array_creation_expression) =
  (match x with
  | `Array_LPAR_opt_array_elem_init_rep_COMMA_array_elem_init_opt_COMMA_RPAR (v1, v2, v3, v4, v5) ->
      ...
      todo env (v1, v2, v3, v4, v5)
  | `LBRACK_opt_array_elem_init_rep_COMMA_array_elem_init_opt_COMMA_RBRACK (v1, v2, v3, v4) ->
      let v1 = (* "[" *) token env v1 in
      let v2 =
        (match v2 with
        | Some x ->
...
and map_callable_variable (env : env) (x : CST.callable_variable) =
  ...
  | `Func_call_exp (v1, v2) ->
      let v1 =
        ...
        | `Rese_id x -> map_reserved_identifier env x
        | `Qual_name x -> map_qualified_name env x
        | `Choice_choice_choice_dyna_var_name x ->
...
and map_class_type_designator (env : env) (x : CST.class_type_designator) =
  ...
  | `Null_member_access_exp x ->
      map_nullsafe_member_access_expression env x
  | `Scoped_prop_access_exp x ->
...
and map_const_declaration_ (env : env) ((v1, v2, v3, v4, v5) : CST.const_declaration_) =
  let v1 =
    ...
    | None -> todo env ())
  in
  let v2 = map_pat_7beed81 env v2 in
...
and map_else_if_clause (env : env) ((v1, v2, v3) : CST.else_if_clause) =
  let v1 = map_pat_66accd2 env v1 in
...
and map_expression (env : env) (x : CST.expression) =
  ...
  | `Cond_exp (v1, v2, v3, v4, v5) ->
      ...
      todo env (v1, v2, v3, v4, v5)
  | `Match_exp (v1, v2, v3) ->
      let v1 = map_pat_7f9f785 env v1 in
      let v2 = map_parenthesized_expression env v2 in
  ...
  | `Incl_exp (v1, v2) ->
      let v1 = map_pat_5295da4 env v1 in
      let v2 = map_expression env v2 in
      todo env (v1, v2)
  | `Incl_once_exp (v1, v2) ->
      let v1 = map_pat_8b02371 env v1 in
      let v2 = map_expression env v2 in
      todo env (v1, v2)
  | `Requ_exp (v1, v2) ->
      let v1 = map_pat_35b755d env v1 in
      let v2 = map_expression env v2 in
      todo env (v1, v2)
  | `Requ_once_exp (v1, v2) ->
      let v1 = map_pat_feb296c env v1 in
      let v2 = map_expression env v2 in
      todo env (v1, v2)
  )
...
and map_function_definition_header (env : env) ((v1, v2, v3, v4, v5) : CST.function_definition_header) =
  let v1 = map_pat_174c3a5 env v1 in
  let v2 =
    (match v2 with
    | Some tok -> (* "&" *) token env tok
    | None -> todo env ())
...
and map_member_declaration (env : env) (x : CST.member_declaration) =
  ...
  | `Class_const_decl (v1, v2, v3) ->
      ...
      let v2 =
        (match v2 with
        | Some x -> map_final_modifier env x
        | None -> todo env ())
      in
...
and map_object_creation_expression (env : env) (x : CST.object_creation_expression) =
  ...
  | `New_choice_qual_name_opt_args (v1, v2, v3) ->
      ...
      in
      todo env (v1, v2, v3)
  | `New_pat_a7a1629_opt_args_opt_base_clause_opt_class_inte_clause_decl_list (v1, v2, v3, v4, v5, v6) ->
      let v1 = (* "new" *) token env v1 in
...
and map_primary_expression (env : env) (x : CST.primary_expression) =
  ...
  | `Anon_func_crea_exp (v1, v2, v3, v4, v5, v6, v7) ->
      let v1 =
        (match v1 with
        | Some x -> map_static_modifier env x
        | None -> todo env ())
      in
      let v2 = map_pat_174c3a5 env v2 in
      let v3 =
        (match v3 with
        | Some tok -> (* "&" *) token env tok
        | None -> todo env ())
  ...
  | `Arrow_func (v1, v2, v3, v4, v5, v6, v7) ->
      let v1 =
        ...
        | None -> todo env ())
      in
      let v2 = map_pat_fb2c292 env v2 in
      let v3 =
        (match v3 with
        | Some tok -> (* "&" *) token env tok
        | None -> todo env ())
...
and map_statement (env : env) (x : CST.statement) =
  ...
  | `If_stmt (v1, v2, v3) ->
      ...
      let v3 =
        ...
        | `Choice_empty_stmt_rep_else_if_clause_opt_else_clause (v1, v2, v3) ->
            ...
            in
            todo env (v1, v2, v3)
        | `Colon_blk_rep_else_if_clause_2_opt_else_clause_2_pat_b10beb6_choice_auto_semi (v1, v2, v3, v4, v5) ->
            ...
            let v3 =
              ...
              | None -> todo env ())
            in
            let v4 = map_pat_b10beb6 env v4 in
            let v5 = map_semicolon env v5 in
            todo env (v1, v2, v3, v4, v5)
        )
      in
  ...
  | `While_stmt (v1, v2, v3) ->
      ...
      let v3 =
        ...
        | `Colon_blk_pat_2bdb730_choice_auto_semi (v1, v2, v3) ->
            let v1 = map_colon_block env v1 in
            let v2 = map_pat_2bdb730 env v2 in
  ...
  | `For_stmt (v1, v2, v3, v4, v5, v6, v7, v8, v9) ->
      ...
      let v9 =
        ...
        | `Choice_auto_semi x -> map_semicolon env x
        | `Choice_empty_stmt x -> map_statement env x
        | `COLON_rep_choice_empty_stmt_pat_1d5f5b3_choice_auto_semi (v1, v2, v3, v4) ->
            let v1 = (* ":" *) token env v1 in
            let v2 = List.map (map_statement env) v2 in
            let v3 = map_pat_1d5f5b3 env v3 in
  ...
  | `Fore_stmt (v1, v2, v3, v4, v5, v6, v7) ->
      let v1 = map_pat_de4334b env v1 in
      let v2 = (* "(" *) token env v2 in
      let v3 = map_expression env v3 in
      let v4 = map_pat_79b8891 env v4 in
      let v5 =
        (match v5 with
      ...
      let v7 =
        ...
        | `Colon_blk_pat_25e0188_choice_auto_semi (v1, v2, v3) ->
            let v1 = map_colon_block env v1 in
            let v2 = map_pat_25e0188 env v2 in
            let v3 = map_semicolon env v3 in
            todo env (v1, v2, v3)
        )
      in
      todo env (v1, v2, v3, v4, v5, v6, v7)
  | `Goto_stmt (v1, v2, v3) ->
      let v1 = map_pat_7f63d6a env v1 in
      let v2 =
        (* pattern [_a-zA-Z\u00A1-\u00ff][_a-zA-Z\u00A1-\u00ff\d]* *) token env v2
      in
      let v3 = map_semicolon env v3 in
  ...
  | `Decl_stmt (v1, v2, v3, v4, v5) ->
      ...
      let v5 =
        (match v5 with
        | `Choice_empty_stmt x -> map_statement env x
        | `COLON_rep_choice_empty_stmt_pat_bb9603f_choice_auto_semi (v1, v2, v3, v4) ->
            let v1 = (* ":" *) token env v1 in
            let v2 = List.map (map_statement env) v2 in
            let v3 = map_pat_bb9603f env v3 in
            let v4 = map_semicolon env v4 in
            todo env (v1, v2, v3, v4)
        | `Choice_auto_semi x -> map_semicolon env x
        )
      ...
      todo env (v1, v2, v3, v4, v5)
  | `Echo_stmt (v1, v2, v3) ->
      let v1 = map_pat_40e76f1 env v1 in
  ...
  | `Inte_decl (v1, v2, v3, v4) ->
      let v1 = map_pat_1f94420 env v1 in
      let v2 =
        (* pattern [_a-zA-Z\u00A1-\u00ff][_a-zA-Z\u00A1-\u00ff\d]* *) token env v2
      in
      let v3 =
      ...
      todo env (v1, v2, v3, v4)
  | `Trait_decl (v1, v2, v3) ->
      let v1 = map_pat_3e83199 env v1 in
      let v2 =
        (* pattern [_a-zA-Z\u00A1-\u00ff][_a-zA-Z\u00A1-\u00ff\d]* *) token env v2
      in
  ...
  | `Global_decl (v1, v2, v3, v4) ->
      let v1 = map_pat_f5e99ef env v1 in
      let v2 = map_variable_name_ env v2 in
      let v3 =
        List.map (fun (v1, v2) ->
          let v1 = (* "," *) token env v1 in
...
and map_switch_block (env : env) (x : CST.switch_block) =
  ...
  | `LCURL_rep_choice_case_stmt_RCURL (v1, v2, v3) ->
      let v1 = (* "{" *) token env v1 in
      let v2 =
        List.map (map_anon_choice_case_stmt_f1b35bc env) v2
      in
      let v3 = (* "}" *) token env v3 in
      todo env (v1, v2, v3)
  | `COLON_rep_choice_case_stmt_pat_0b47e00_choice_auto_semi (v1, v2, v3, v4) ->
      ...
      let v2 =
        List.map (map_anon_choice_case_stmt_f1b35bc env) v2
      in
      let v3 = map_pat_0b47e00 env v3 in
      let v4 = map_semicolon env v4 in
      todo env (v1, v2, v3, v4)
  )
...
and map_use_as_clause (env : env) ((v1, v2, v3) : CST.use_as_clause) =
  ...
  in
  let v2 = map_pat_79b8891 env v2 in
  let v3 =
    (match v3 with
...
and map_use_instead_of_clause (env : env) ((v1, v2, v3) : CST.use_instead_of_clause) =
  let v1 =
    map_anon_choice_class_cst_access_exp_18f5288 env v1
  in
  let v2 = map_pat_5641180 env v2 in
  let v3 =
    (* pattern [_a-zA-Z\u00A1-\u00ff][_a-zA-Z\u00A1-\u00ff\d]* *) token env v3
  in
  todo env (v1, v2, v3)
